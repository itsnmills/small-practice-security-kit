# Velari Practice Assurance Packet Offering Blueprint

## Positioning

Velari Practice Assurance Packet for Small Dental Practices is a plain-English security and vendor evidence report for practice owners, office managers, MSPs, vendor reviewers, and legal/compliance reviewers.

The promise is practical: start with a 10-minute intake, add safe public-site observations, map where patient-data workflows create risk outside the EHR, show which evidence is missing, turn guidance into questions the practice can ask this week, and separate owner/MSP/vendor/legal lanes without moving PHI or raw evidence into the public repo. The deliverable is a report and evidence handoff, not a generic dashboard the practice has to manage.
...
- HHS/OCR Online Tracking Technologies Guidance: https://www.hhs.gov/hipaa/for-professionals/privacy/guidance/hipaa-online-tracking/index.html
  - Sprint implication: treat public-site tracker observations on scheduler, intake, portal, payment, registration, and similar workflows as qualified-review questions, not automatic legal conclusions.
- HHS HIPAA Security Rule Summary: https://www.hhs.gov/hipaa/for-professionals/security/laws-regulations/index.html
  - Sprint implication: route transmission, access-control, integrity, audit, and person-or-entity authentication evidence to the owner/MSP/reviewer lanes.

## Included Scope

- Intake safety boundary and owner/MSP lane assignment.
- 10-minute intake checklist for owner, vendors, public URLs, AI tools, and MSP evidence status.
- External Evidence Pre-Check for authorized public-site tracker, scheduler, portal, TLS, and certificate observations.
...
- Buyer-facing `practice-assurance-packet.html` report.
- Plain Markdown copy in `practice-assurance-packet.md`.
- Reference-only `external-evidence-precheck.md` report.
...
- Formal Security Risk Analysis, legal/compliance conclusions, incident reporting decisions, cyber-insurance advice, penetration testing, unauthorized vulnerability scanning, MDR, SOC, forensic work, live system verification, vendor acceptance, or AI production-use authorization.
...
3. Capture any authorized, reference-only external observations without submitting real patient forms or storing sensitive intercepted payloads.
4. Build Sprint Mode packet:

   ```bash
   python3 -m small_practice_security_kit sprint samples/family_dental_clinic.yaml --output-root out
   ```

5. Start with `practice-assurance-packet.html`.
6. Use `external-evidence-precheck.md` to route tracker, scheduler, portal, TLS, and certificate questions to the website vendor, MSP, and qualified reviewer.
7. Use `owner-action-plan.md` for the first 7 days.
8. Send `msp-remediation-brief.md` to the MSP for evidence references and remediation sequence.
9. Send `vendor-baa-ai-questionnaire.md` to vendor owners or qualified reviewers.
10. Use `evidence-collection-checklist.md` to gather proof in the private/offline binder.
11. Use `source-map.md` to explain how HHS/HICP/CISA/ONC-OCR anchors shaped the questions.
12. Import `sprint-summary.json`, `evidence-index.json`, `risk-register.csv`, and `handoff-actions.csv` into the private app only after review.

## Artifacts

| Artifact | Purpose |
|---|---|
| `practice-assurance-packet.html` | Polished buyer-facing report: executive snapshot, top risks, audience handoffs, evidence requests, MSP value, and limitations. |
| `practice-assurance-packet.md` | Plain Markdown copy of the assurance packet, including 10-minute intake and owner decision queue. |
| `external-evidence-precheck.md` | Reference-only public-site tracker, scheduler, portal, TLS, and certificate observations translated into safe questions. |
...
- External precheck artifacts may contain page/workflow labels, tag or script names, destination domains, TLS/certificate status summaries, and owner questions, but not real patient-submitted values, session cookies, or full network payloads.
...
- Involve the MSP or website host when HTTPS redirect behavior, certificate validity, TLS posture, HSTS status, or host ownership is unclear.
...
- Involve legal/compliance reviewers when contract interpretation, formal risk assessment, incident reporting, insurance, AI rollout, vendor data use, tracker/pixel posture on patient-facing workflows, or policy acceptance decisions are needed.
