...
RULES = [
    ...
    ("ssn_label", "high", re.compile(r"(?i)\b(ssn|social security number)\b\s*[:=]?\s*\d{3}[- ]?\d{2}[- ]?\d{4}\b"), "Looks like an SSN."),
    ...
    ("labeled_secret", "high", re.compile(r"(?i)\b(api[_ -]?key|access[_ -]?key|token|secret)\b\s*[:=]\s*['\"]?[A-Za-z0-9_./+=-]{8,}"), "Looks like a credential."),
    ...
    ("patient_name_label", "high", re.compile(r"(?i)\bpatient(?:\s+name\s*[:=]?|\s*[:=])\s*[A-Za-z][A-Za-z'.-]+"), "Looks like a patient name label."),
    ...
    ("private_url", "high", re.compile(r"(?i)https?://[^\s]+(?:[?&](?:token|secret|signature|sig|key|password|passcode)=|X-Amz-Signature|presigned|private)[^\s]*"), "Looks like a private or signed URL."),
    ("bearer_or_basic_auth", "high", re.compile(r"(?i)\b(authorization:\s*)?(bearer|basic)\s+(?=[A-Za-z0-9._~+/=-]*\d)[A-Za-z0-9._~+/=-]{12,}"), "Looks like an authorization credential."),
]
...
def _walk(value: Any, path: str = "") -> list[tuple[str, str]]:
    # Security decision: dict keys and non-string scalars (YAML ints, dates)
    # can carry PHI too, so keys are scanned with their scalar values joined
    # as "key: value" text.
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if isinstance(child, (dict, list)):
                ...
                items.append((child_path, f"{key}: {scalar}"))
